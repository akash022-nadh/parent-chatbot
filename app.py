...
# Import the new Flask application factory
...
if __name__ == "__main__":
    ...
    # Check if MongoDB has data, if not initialize
    try:
        client = MongoClient(MONGO_URI)
        database = client[DB_NAME]
        if database.students.count_documents({}) == 0:
            print("No data found in MongoDB. Initializing sample data...")
            init_db()
        client.close()
    except Exception as e:
        print(f"MongoDB connection error: {e}")
        print("Please ensure MongoDB is running locally or set MONGO_URI environment variable")
...
║    • Role-based Access (Admin, Faculty, Parent, Student)    ║
║    • REST API Endpoints                                       ║
║    • Academic Dashboards                                      ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
...
@app.route("/")
def index():
    return "Parent Chatbot is running successfully!"
